...
jQuery(function ($) {
    ...
    const $results_shortcode_filters_wrapper = $('#results_shortcode_filters_wrapper')
    const $results_shortcode_filters = $('#results_shortcode_filters')
    const $results_shortcode = $('#results_shortcode')
    ...
    function generateShortCode() {
        let post_type_val = $('#post_type_load_more').find(':selected').val(),
            ...
            shortCodeArray = [],
            shortCodeFiltersArray = [],
            shortCodeFilterId = '';

        shortCodeFilterId = 'filter_id="' + post_type_val + '_filter"';
        shortCodeArray.push(
            ' post_type="' +
            post_type_val +
            '"' +
        ...
        if ($('#filter_by_category').is(':checked')) {
            ...
            if (filter_taxonomy_val !== '') {
                shortCodeFiltersArray.push(' filter_taxonomy="' + filter_taxonomy_val + '"');
            }
            if (filter_type_val !== '') {
                shortCodeFiltersArray.push(' filter_type="' + filter_type_val + '"');
            }
            if ($('#enable_filter_titles').is(':checked')) {
                filter_titles = ' filter_titles="true"';
            }
            if ($('#multiply_filter').is(':checked')) {
                multiply_filter = ' multiply_filter="true"';
            }
        ...
        if ($('#enable_order').is(':checked')) {
            ...
            if (label_old_order_val !== '') {
                shortCodeFiltersArray.push(' label_old_order="' + label_old_order_val + '"');
            }
        } else {
            $('.hiddenOrder').hide();
        }
        ...
        $results_shortcode.val(
            '[all_posts_ajax' +
            shortCodeArray + ' ' + shortCodeFilterId +
            ']'
        );

        shortCodeFiltersArray = shortCodeFiltersArray.join('');
        shortCodeFiltersArray = shortCodeFiltersArray.toString();
        if (filter_by_category || enable_search || enable_order) {
            $results_shortcode_filters_wrapper.fadeIn();

            $results_shortcode_filters.val(
                '[all_posts_ajax_filters' +
                shortCodeFiltersArray +
                filter_by_category +
                multiply_filter +
                enable_clear_button +
                enable_search +
                enable_order +
                filter_titles + ' ' + shortCodeFilterId +
                ']'
            );
        } else {
            $results_shortcode_filters_wrapper.fadeOut();
        }
